...
public partial class Card : Node3D
{
    ...
    {
        var cardName = GetNode<Label>("/root/Card/FrontFace/Name/Name");
        cardName.Text = cards[req]["Name"];
        var cardRank = GetNode<Label>("/root/Card/FrontFace/Name/Rank");
        cardRank.Text = cards[req]["Rank"];
        var cardDescription = GetNode<Label>("/root/Card/FrontFace/Description/Description");
        cardDescription.Text = cards[req]["Description"];
        var cardStats = GetNode<Label>("/root/Card/FrontFace/Description/Stats");
        cardStats.Text = cards[req]["ATK"] + " ATK / " + cards[req]["HP"] + " HP";
        var newImage = ImageTexture.CreateFromImage(getImg(cards[req]["Image"]));
        var mesh = GetNode<MeshInstance3D>("/root/Card/FrontFace/Picture");
        ...
        if (cards[req]["Type"])
        {
            ...
            var meshBg = GetNode<MeshInstance3D>("/root/Card/FrontFace");
